...
chatAE =
    item "chat-ae"
        "AmeliaとEmmaのチャット"
        [ script
            [ emma "「これからお昼に行こうと思うんだけど、一緒にどう？」"
            , amelia "「ごめんなさい。今はあの子の観察を続けたいの。」"
            , emma "「随分と熱心ね。前はああいうの苦手じゃなかった？」"
            , amelia "「そうね。でも最近は興味が湧いてきて、目が離せないの。」"
            , emma "「そう、わかったわ。食事はまた別の機会に誘うわね。」"
            , amelia "「ええ、ありがとう、Emma。」"
            ]
        ]
...
chatCE =
    item "chat-ce"
        "CarloとEmmaのチャット"
        [ script
            [ carlo "「最近のLiaは一体どうしたんだ？あれの前に張り付いてばかりじゃないか？」"
            , emma "「ええ、なんだか随分と熱が入っているみたいね。」"
            , carlo "「熱心なんてもんか！あれじゃまるで恋する乙女だ！！」"
            , emma "「妬いてるの？あんなものに嫉妬なんて男の器が知れるわよ？」"
            , carlo "「俺は心配なだけさ。可憐な花がただ虫と戯れるだけに一生を終えるのかと。」"
            , emma "「心配ならBenにしてやりなさいよ。彼、最近妙におどおどしてるわよ。」"
            , carlo "「そうか？あいつは前からあんな感じだよ。」"
            , emma "「あなたって、本当にイタリア男ね…」"
            , carlo "「最高の褒め言葉だ。」"
            ]
        ]
...
mailD : Html msg
mailD =
    item "mail-d" "Davidから各研究員へのメール内容" <|
        mail
            [ [ "研究員各位、" ]
...
mailBD : Html msg
mailBD =
    item "mail-bd" "BenjaminとDavidのメール内容" <|
        [ mailBD1
        , mailBD2
        ]
...
plansec : String -> List (Html msg) -> Html msg
plansec heading =
    (::) (E.h3 [] [ text heading ])
        >> E.section []
...
item : String -> String -> List (Html msg) -> Html msg
item id name =
    (::) (E.h2 [] [ text name ])
        >> E.section [ A.id id ]


subitem : String -> List (Html msg) -> Html msg
subitem name =
    (::) (E.h3 [] [ text name ])
        >> E.section []


script : List (Html msg) -> Html msg
script =
    E.div [ A.class "script" ]


amelia : String -> Html msg
amelia =
    line "5" "Amelia"


carlo : String -> Html msg
carlo =
    line "4" "Carlo"


emma : String -> Html msg
emma =
    line "3" "Emma"
...
mail : List (List String) -> List (Html msg)
mail =
    List.map text
